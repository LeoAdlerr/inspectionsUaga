...
CREATE TABLE `users` (
  ...
  `is_active` TINYINT(1) NOT NULL DEFAULT 1 COMMENT 'Status do usuario. 1 para ativo, 0 para inativo.',
  `signature_path` VARCHAR(512) NULL DEFAULT NULL COMMENT 'Caminho para o arquivo de assinatura pessoal do usuario.',
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX `idx_users_username` (`username`)
) ENGINE=InnoDB COMMENT='Tabela central para armazenamento de usuarios do sistema.';
...
CREATE TABLE `roles` (
  ...
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='Define as funcoes (perfis) de usuario no sistema (RBAC).';
...
) ENGINE=InnoDB COMMENT='Tabela de juncao para associar usuarios a funcoes (N:M).';
...
CREATE TABLE `inspections` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `inspector_id` INT NULL COMMENT 'FK para o usuario que realizou a inspecao. Nulo ate que um inspetor assuma.', -- ALTERACAO AQUI: A coluna inspector_id agora e NULL.
