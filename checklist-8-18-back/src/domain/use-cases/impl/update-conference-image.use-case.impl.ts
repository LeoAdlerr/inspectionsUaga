...
import { UpdateConferenceImageUseCase } from '../update-conference-image.use-case';
...
const IMAGE_CATEGORY_PANORAMIC = 2; // Fotos do Conferente

@Injectable()
export class UpdateConferenceImageUseCaseImpl implements UpdateConferenceImageUseCase {
  private readonly logger = new Logger(UpdateConferenceImageUseCaseImpl.name);
  ...
  async execute(
    ...
    // GUARDRAIL: Conferente só edita Panorâmicas
    if (image.categoryId !== IMAGE_CATEGORY_PANORAMIC) {
      throw new ForbiddenException('O conferente só pode editar fotos panorâmicas.');
    }
    ...
    if (file) {
      ...
      if (image.photoPath) {
        ...
        const oldAbsPath = path.join(process.cwd(), 'uploads', image.photoPath);
        try {
        ...
        baseName = `conference_image_${imageId}`;
      }
